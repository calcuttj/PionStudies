...
int effPur_eventSelection_francesco(const string mcFilepath){
   ...
   //output file
   TFile *output = new TFile ("eSliceMethod_effPur_binByBin_francesco.root", "RECREATE");
...
//test
   ...
   selected_incidentPion_truePion //N true incident pions after incident pion selection
      .Foreach( [h_sel_truePi_trueE_equal_recoE] (double true_int, double reco_int, double reco_inc) {
            //bin that energy falls into is (int) energy/nbins + 1
            //
            int true_bin_interacting = (int) true_int / bin_size_int + 1;
            int reco_bin_interacting = (int) reco_int / bin_size_int + 1;
            int reco_bin_incident = (int) reco_inc / bin_size_int + 1;

            if(true_bin_interacting == reco_bin_interacting){
            ...
            ,{ "true_KEint_fromEndP", "reco_interactingKE", "reco_firstEntryIncident"});
